<?php
    //$name = 's1901037';//テスト用。結合テスト終了後は消す。
    ...
    $grade = $row['grade'];
    //教科のテーブル
    $SQL_sub="SELECT * FROM SubGrade WHERE Grade = $grade";
    $stmt_sub = $pdo->query($SQL_sub);
    $row_sub = $stmt_sub->fetchAll(PDO::FETCH_ASSOC);
    //教科テーブルのレコード長取得
    $SQL_length = "SELECT COUNT(*) FROM SubGrade WHERE Grade = $grade";
...
<html lang = " ja ">
    <head>
        <title>教科選択[<?php echo(htmlspecialchars($grade))?>年生]</title>
        <meta charset="UTF-8">
        <!--CSS-->
        <style>
            *{
            ...
            footer a:hover,nav a:hover,.SubjectSelect ul a:hover{
                color: #F66496;
	            font-size: 22px;
	            font-weight: bold;
            }
            footer a,nav a,.SubjectSelect ul a{
                color: #E66496;
                font-size: 20px;
                margin: 10pt;
            }
    ...
    <body>
        <div id="Face">
            <h1 style="margin: 0 10px">教科選択[<?php echo(htmlspecialchars($grade))?>学年]</h1>
        </div>
        <nav><!--左側-->
        <div id="GradeChange">
            <form name="fGC" method="POST" action="PBL_SubjectSelect_AllGrade.php">
                <input type=hidden name=name value=<?php echo $name;?>>
            </form>
            <a href="javascript:document.fGC.submit()">➣全学年版</a></div><div class="UnderLine">
        </div>
        <div style="margin: 0 10pt 0;">同学年版</div>
        </nav>
        <div id="subList"><!--右側-->
